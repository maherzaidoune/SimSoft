using System;
namespace PFE.Models
{
    public class PIECEACHAT
    {
        public int? PCAID { get; set; }
        public string PCANUM { get; set; }
        public string PCANUMEXT { get; set; }
        public string PITCODE { get; set; }
        public int? PINID { get; set; }
        public string PINCODE { get; set; }
        public int? EXEID { get; set; }
        public int? NUMID { get; set; }
        public int? AFFID { get; set; }
        public int? TIRID { get; set; }
        public int? TIRID_FAC { get; set; }
        public int? ADRID_FAC { get; set; }
        public int? ADRID_LIV { get; set; }
        public int? DEPID { get; set; }
        public int? EXPID { get; set; }
        public int? PCACOLISAGE { get; set; }
        public int? PCAPOIDS { get; set; }
        public int? PCAUNITEPOIDS { get; set; }
        public int? JORID { get; set; }
        public int? ECRSEQUENCE { get; set; }
        public int? DEVID { get; set; }
        public int? PCACOURSDEV { get; set; }
        public DateTime? PCADATEEFFET { get; set; }
        public string PCAISSOLDE { get; set; }
        public string PCAISCOMPTABILISE { get; set; }
        public string PCAISCLOS { get; set; }
        public string PCAISPRINT { get; set; }
        public int? PCANBPRINT { get; set; }
        public DateTime? PCADATEPRINT { get; set; }
        public int? MODID { get; set; }
        public float? PCAMNTHT { get; set; }
        public float? PCAMNTTTC { get; set; }
        public int? PCAMNTAREGLER { get; set; }
        public int? PCAMNTACOMPTE { get; set; }
        public float? PCAMNTTVA { get; set; }
        public int? PCAMNTTPF { get; set; }
        public int? PCAMNTESCOMPTE { get; set; }
        public int? PCAMNTPORT { get; set; }
        public double? PCATAUXESCOMPTE { get; set; }
        public string PCACONDREGLEMENT { get; set; }
        public string USRMODIF { get; set; }
        public DateTime? DATEUPDATE { get; set; }
        public DateTime? DATECREATE { get; set; }
        public int? MEMOID { get; set; }
        public int? PCANBIMPRESSION { get; set; }
        public int? SOCID { get; set; }
        public string PCAISDEB { get; set; }
        public string PCADEBREGIME { get; set; }
        public string PCADEBTRANSACTION { get; set; }
        public string PCADEBLIVRAISON { get; set; }
        public string PCADEBTRANSPORT { get; set; }
        public int? PCAVOLUME { get; set; }
        public int? PCAUNITEVOLUME { get; set; }
        public string PCAISPIECEFRAIS { get; set; }
        public int? ANSID { get; set; }
        public DateTime? PCADATELIVRAISON { get; set; }
        public string USRCREATE { get; set; }
        public string PCAOBJET { get; set; }
        public string TYNCODE { get; set; }
        public int? TIRID_CPT { get; set; }
    }
}
